...
def accumulate_events(event_parameters: List[Any], n_events=2000):
    params_per_set = n_events * 4
    n_complete_sets = len(event_parameters) // params_per_set
    all_polarity = []
    negative_polarity = []
    positive_polarity = []

    for i in range(0, n_complete_sets * params_per_set, params_per_set):
        event_set = event_parameters[i:i + params_per_set]
